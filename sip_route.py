...
class A2PBlockerTab:
    ...
    def _setup_ui(self):
        ...
        lbl_add = ttk.Label(right_frame, text="Nhập số cần CHẶN THÊM (Mỗi dòng 1 số; tự cắt 0, 84 ở đầu):", font=self.bold_font)
        ...
        lbl_del = ttk.Label(right_frame, text="Nhập số cần GỠ CHẶN (chỉ gỡ số đang chặn):", font=self.bold_font)
    ...
    def analyze_and_generate(self):
        ...
        def _normalize_pair(num):
            # Cắt 84 đầu (nếu có) hoặc 0 đầu (nếu có) -> số gốc N; trả về duy nhất N.
            n = num.strip()
            if n.startswith("84"): n = n[2:]
        ...
        def _parse_pairs(raw):
            ...
            for l in raw.splitlines():
                s = l.strip()
                if not s or s.startswith("#") or not s.isdigit(): continue
                out.update(_normalize_pair(s))
            return sorted(out)

        raw_add = self.txt_new_block.get("1.0", "end").strip()
        input_add = _parse_pairs(raw_add)

        raw_del = self.txt_priority_del.get("1.0", "end").strip()
        if raw_del == self.del_placeholder.strip(): raw_del = ""
        input_del = _parse_pairs(raw_del)
...
class SipRouterApp:
    def __init__(self, root):
        ...
        out_header.pack(fill=X, side=TOP)
        self.btn_toggle_edit_cmd = ttk.Button(out_header, text="🔓 Cho phép sửa", command=self.toggle_edit_commands, bootstyle="warning")
        self.btn_toggle_edit_cmd.pack(side=RIGHT, padx=2, pady=(0, 2))
    ...
    def _get_uctt_status_from_node(self, host, user, pw, node_name, selected_rc_raw, check_op_cmds, check_nop_cmds):
        ...
        try:
            ...
            for i, rc_item in enumerate(rc_list):
                ...
                op_output = self._read_until_prompt_or_end(shell, ["<"], timeout=15, node=node_name)
                ...
                nop_output = self._read_until_prompt_or_end(shell, ["<"], timeout=15, node=node_name)
    ...
    def toggle_edit_commands(self):
        """Bật/Tắt chế độ cho phép sửa tay ô 'Lệnh MML sẽ thực thi'."""
        is_disabled = str(self.txt_output_commands.cget('state')) == 'disabled'
        if is_disabled:
            self.txt_output_commands.configure(state='normal', bg="#FFF3CD")
            self.btn_toggle_edit_cmd.config(text="🔒 Khóa lại", bootstyle="danger")
        else:
            self.reset_edit_state_commands()

    def reset_edit_state_commands(self):
        """Đưa ô 'Lệnh MML sẽ thực thi' và nút toggle về trạng thái khóa mặc định."""
        self.txt_output_commands.configure(state='disabled', bg="#F8F9FA")
        self.btn_toggle_edit_cmd.config(text="🔓 Cho phép sửa", bootstyle="warning")
    ...
    def start_automation_both(self):
        ...
        if not is_check_job: 
            ...
            edit_warning = "⚠️ LỆNH ĐÃ ĐƯỢC CHỈNH SỬA TAY!\n\n" if str(self.txt_output_commands.cget('state')) == 'normal' else ""
            confirm = messagebox.askyesno(
                "Xác nhận Cấu hình",
                f"{edit_warning}Bạn có chắc muốn chạy {len(commands_to_run)} lệnh CẤU HÌNH (Route/Delete) trên CẢ HAI node?\n\n"
    ...
    def start_automation_thread(self, device_name):
        ...
        if not is_check_job: 
            ...
            confirm = messagebox.askyesno(
                "Xác nhận Cấu hình",
                f"{edit_warning}Bạn có chắc muốn chạy {len(commands_to_run)} lệnh CẤU HÌNH (Route/Delete) trên node {device_name}?\n\n"
                f"Các lệnh sẽ chạy (ví dụ):\n{commands_display}"
            )
            if not confirm:
